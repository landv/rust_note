...
# 测试TCP代理网络性能的脚本
# 命名空间。创建三个命名空间，左、右、代理
# 设置一个客户端在左，代理在中间，以及服务器在右。
...
function benchmark() {
    ip netns exec proxy /media/landv/系统/landv/rust_note/tcp_proxy/target/debug/proxy -c 0.0.0.0:1212 -s 10.0.0.3:1313 &
